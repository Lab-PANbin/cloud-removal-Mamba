...
dir_checkpoint = Path('./CR-Famba_Model/')
...
if opt.TRAINING.RESUME:
    path_chk_rest    = './CR-Famba_Model/model_best.pth'
...
######### Loss ###########
criterion_char = losses.PSNRLoss()
...
val_loader = DataLoader(dataset=val_dataset, batch_size=8, shuffle=False, num_workers=8, drop_last=False, pin_memory=True)
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        loss_edge = 0
        loss_char = 0
        loss = criterion_char(restored, target)
    ...
    if epoch % opt.TRAINING.VAL_AFTER_EVERY == 0:
        ...
        print("te", psnr_te)
        # print("[teps:%f,,,,,,ssim:%f]" % (psnr_te_1, ssim_te_1))
